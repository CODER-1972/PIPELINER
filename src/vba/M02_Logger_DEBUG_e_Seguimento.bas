...
Public Sub Debug_Registar( _
    ...
    Debug_SetValue ws, mapa, novaLinha, "Parametro", parametro          ' aceita "ParÃ¢metro" no Excel
    Debug_SetValue ws, mapa, novaLinha, "Problema", problema
    Debug_SetValue ws, mapa, novaLinha, "Sugestao", sugestao            ' aceita "SugestÃ£o" no Excel
End Sub
...
Private Function Debug_RemoverAcentos(ByVal s As String) As String
    ...
    ' a
    s = Replace(s, ChrW(225), "a") ' Ã¡
    s = Replace(s, ChrW(224), "a") ' Ã 
    s = Replace(s, ChrW(226), "a") ' Ã¢
    s = Replace(s, ChrW(227), "a") ' Ã£
    s = Replace(s, ChrW(228), "a") ' Ã¤

    ' e
    s = Replace(s, ChrW(233), "e") ' Ã©
    s = Replace(s, ChrW(232), "e") ' Ã¨
    s = Replace(s, ChrW(234), "e") ' Ãª
    s = Replace(s, ChrW(235), "e") ' Ã«

    ' i
    s = Replace(s, ChrW(237), "i") ' Ã­
    s = Replace(s, ChrW(236), "i") ' Ã¬
    s = Replace(s, ChrW(238), "i") ' Ã®
    s = Replace(s, ChrW(239), "i") ' Ã¯

    ' o
    s = Replace(s, ChrW(243), "o") ' Ã³
    s = Replace(s, ChrW(242), "o") ' Ã²
    s = Replace(s, ChrW(244), "o") ' Ã´
    s = Replace(s, ChrW(245), "o") ' Ãµ
    s = Replace(s, ChrW(246), "o") ' Ã¶

    ' u
    s = Replace(s, ChrW(250), "u") ' Ãº
    s = Replace(s, ChrW(249), "u") ' Ã¹
    s = Replace(s, ChrW(251), "u") ' Ã»
    s = Replace(s, ChrW(252), "u") ' Ã¼

    ' c
    s = Replace(s, ChrW(231), "c") ' Ã§
...
Public Sub Seguimento_Registar( _
    ...
    If mapa.exists("Config extra (amigÃ¡vel)") Then ws.Cells(linha, mapa("Config extra (amigÃ¡vel)")).value = prompt.ConfigExtra
...
Private Function SegHTTPDesc(ByVal code As Long) As String
    Select Case code
        Case 200: SegHTTPDesc = "200 OK â€” Sucesso"
        Case 201: SegHTTPDesc = "201 â€” Criado"
        Case 400: SegHTTPDesc = "400 â€” Pedido invÃ¡lido"
        Case 401: SegHTTPDesc = "401 â€” NÃ£o autorizado"
        Case 403: SegHTTPDesc = "403 â€” Proibido"
        Case 404: SegHTTPDesc = "404 â€” NÃ£o encontrado"
        Case 429: SegHTTPDesc = "429 â€” Limite atingido"
        Case 500: SegHTTPDesc = "500 â€” Erro interno"
        Case 502: SegHTTPDesc = "502 â€” Bad gateway"
        Case 503: SegHTTPDesc = "503 â€” IndisponÃ­vel"
        Case 504: SegHTTPDesc = "504 â€” Timeout"
        Case Else
            If code >= 200 And code < 300 Then SegHTTPDesc = CStr(code) & " â€” Sucesso (2xx)": Exit Function
            If code >= 300 And code < 400 Then SegHTTPDesc = CStr(code) & " â€” Redirecionamento (3xx)": Exit Function
            If code >= 400 And code < 500 Then SegHTTPDesc = CStr(code) & " â€” Erro do pedido (4xx)": Exit Function
            If code >= 500 And code < 600 Then SegHTTPDesc = CStr(code) & " â€” Erro do servidor (5xx)": Exit Function
...
'   - se exceder: guarda output completo em disco e divide por mÃºltiplas linhas
' ============================================================
...
Private Function Seguimento_LerOutputFolderBase() As String
    ...
    ' Por compatibilidade: OUTPUT Folder estÃ¡ em B3 no layout observado
...
Private Sub Seguimento_EscreverOutputSemTruncagem(ByVal ws As Worksheet, ByVal linhaBase As Long, ByVal mapa As Object, ByVal txtCompleto As String)
    ...
    For i = 1 To n
        ...
        prefixo = "(CONTINUAÃ‡ÃƒO " & CStr(i) & "/" & CStr(n) & ") "
    ...
    Exit Sub
Falha:
    ' fallback: nÃ£o truncar - escreve pelo menos a indicaÃ§Ã£o de erro
    On Error Resume Next
