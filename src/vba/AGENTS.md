...
- Em derivação de nomes para paths de auditoria Git (ex.: `PROMPT_NAME`/`VERSION`), usar parsing explícito do `Prompt ID` (`<folha>/<ordem>/<nome>/<versão>`) e derivar `PROMPT_NAME` no formato `<ordem>_<nome>` (sem prefixo do índice da pipeline); manter fallback por nome da pipeline apenas para localizar o Prompt ID quando o `pipelineIndex` vier inválido.
...
- Em leitura da lista INICIAR no PAINEL para derivar metadados de run (ex.: Prompt ID para pasta Git), não interromper no primeiro vazio; varrer a janela completa e considerar apenas IDs válidos (`<folha>/<ordem>/<nome>/<versão>`), ignorando linhas decorativas/comentários.
