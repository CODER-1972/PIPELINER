...
- Quando o catálogo tiver parâmetros operacionais em `INPUTS:` (ex.: `URLS_ENTRADA`, `MODO_DE_VERIFICACAO`), garantir injeção explícita no texto enviado ao modelo; nunca presumir que `INPUTS` textuais são usados automaticamente só por existirem no layout.
