...
- Em features de escrita em folhas de auditoria opcionais (ex.: Git LOG), nao fixar um unico nome de sheet no runtime; resolver por lista de aliases suportados e registar no DEBUG um preflight explicito (`*_BIND`) + falha de escrita (`*_WRITE`) para diagnosticar rapidamente mismatch de nomenclatura/protecao.
