...
- Em exportações de logs multi-artefacto (catálogo/DEBUG/Seguimento), agrupar sempre os ficheiros numa pasta por execução dentro de `logs/` com nome temporal + pipeline para garantir rastreabilidade, evitar colisões e facilitar rollback/auditoria.
- Em exportações técnicas para repositórios externos, incluir `manifest.json` por execução com estado por artefacto (`ok/http_status/attempts`) para suportar auditoria determinística e troubleshooting sem abrir múltiplos ficheiros.
