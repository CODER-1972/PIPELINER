...
- Quando o catálogo tiver parâmetros operacionais em `INPUTS:` (ex.: `URLS_ENTRADA`, `MODO_DE_VERIFICACAO` e declaração `FILES:`/`FICHEIROS:`), garantir injeção explícita no texto enviado ao modelo; nunca presumir que `INPUTS` são usados automaticamente só por existirem no layout.
- Sempre que houver montagem incremental de prompt (`promptTextFinal`), garantir que todos os consumidores downstream (incluindo M09/Files) recebem essa versão final, evitando divergência entre payload sem anexos e payload com anexos.
...
- Em resolução de `FILES:` com wildcard (`*`), prever fallback de matching flexível (normalização de separadores como `_`/`-`/espaço) quando `Dir` não devolver candidatos, para evitar falsos `NOT_FOUND`.
- Sempre que alterar regras de resolução de `FILES:` (wildcard, `(latest)`, normalização), adicionar ou atualizar um SelfTest automático e determinístico (com pasta temporária + ficheiros dummy) para cobrir regressão sem depender de API/rede.
...
- Para tunning de timeout HTTP no `/v1/responses`, usar chaves de Config (`HTTP_TIMEOUT_RESOLVE_MS`, `HTTP_TIMEOUT_CONNECT_MS`, `HTTP_TIMEOUT_SEND_MS`, `HTTP_TIMEOUT_RECEIVE_MS`) com fallback interno para defaults; validar no DEBUG (`M05_HTTP_TIMEOUTS`) os valores efetivos aplicados.
- Em timeout de transporte no M05, registar no DEBUG um evento dedicado com tipo de timeout provável (`resolve/connect/send/receive/outro`), `elapsed_ms` e snapshot dos `HTTP_TIMEOUT_*_MS` efetivos para reduzir diagnóstico por tentativa/erro.
- Em diagnóstico de timeout HTTP, incluir também a fase (`Open`, `Send`, `Status`, `ResponseText`) e acrescentar fallback (`Err.Number`, `Err.LastDllError`) quando `Err.Description` vier vazio, para evitar registos `[ERRO]` sem detalhe útil.
- Para aumentar precisão de causa em timeout, enriquecer o evento com sinais observáveis (`stage`, `payload_len`, `response_len`, `http_status_partial`) e produzir `cause_hint` + `confidence` + `action` de forma determinística e curta.
- Em timeout HTTP, registar também `started_at`/`failed_at`, contexto de host (`winhttp_proxy`, `vpn_flag`, `host`, `ip_masked`) e `retry_outcome`; isto reduz tempo de triagem entre rede local/proxy e latência do endpoint.
- Para `stage=Send`, aplicar no máximo 1 retry automático com backoff curto e novo socket, registando sempre evento `M05_TIMEOUT_DECISION` com decisão aplicada e decisão sugerida.
- Manter métrica de frequência por `prompt|model` e global na execução (`timeout_count_prompt_model`, `timeout_count_global`) para identificar incidentes sistémicos vs específicos.
- Em handlers com `On Error`, capturar `Err.Number` e `Err.Description` em variáveis locais antes de chamar rotinas de log (`Debug_Registar`) ou alterar modo de erro (`On Error ...`), para evitar perda silenciosa da mensagem original no `resultado.Erro`.
- Em `process_mode=code_interpreter`, quando faltar `container_file_citation`, extrair nomes de ficheiro do `output_text` e usá-los como filtro preferencial sobre a listagem do container; se não houver match, registar ALERTA específico e manter fallback explícito para troubleshooting.
- Na extração de nomes de ficheiro do `output_text` (fallback CI sem citation), suportar marcadores contratuais explícitos (`CI_OUTPUT_FILE:`, `FILE_TSV:`, `OUTPUT_FILE:`) e links `sandbox:/mnt/data/...`, normalizando para basename antes de filtrar a listagem do container.
- Em fallback CI sem citation, registar diagnóstico da qualidade dos marcadores textuais (`raw/valid/invalid`) e alertar quando a shortlist elegível aparenta ficheiros de input (`filename` com prefixo `file-`), para evitar falso sucesso silencioso.
- Em troubleshooting de File Output, distinguir explicitamente "sucesso de transporte HTTP" de "sucesso de contrato de output"; registar sempre uma linha terminal consolidada por passo com impacto e ação recomendada.
- Para correlação M05↔M10, usar fingerprint textual estável (`pipeline|step|prompt|resp|model|ok_http|mode`) nos eventos-chave do DEBUG, sem incluir segredos ou paths sensíveis.
...
- Em módulos com SelfTests, garantir que todos os helpers chamados pelo próprio módulo existem no mesmo módulo ou são `Public`; nunca depender implicitamente de `Private Sub/Function` de outro módulo, porque isso gera `Compile error: Sub or Function not defined` no VBE.
- Na status bar do PAINEL (`Row n de z`), tratar `STOP` como sentinela terminal da lista planeada: a contagem deve parar no primeiro `STOP` e nunca considerar IDs abaixo desse ponto.
- Na status bar do PAINEL (`Step x of y`), o total `y` deve refletir o total planeado da lista ativa (mesma base lógica do `Row n de z`), mantendo `Max Steps` apenas como limite duro de execução.
- Em lookup de `prompt_id` no catálogo (coluna A), aplicar fallback de normalização (remoção de `CR/LF`, `TAB` e `NBSP`) quando a procura exata falhar, para evitar terminação indevida em `STOP` causada por caracteres invisíveis de colagem.
- Em leitura de limites no PAINEL, mapear explicitamente cada parâmetro à sua linha correta (`Max Steps` na linha 5 e `Max Repetitions` na linha 6 no layout atual com `Auto guardar`), evitando reutilizar a mesma célula para ambos por copy/paste.
- Em helpers de UI (ex.: status bar), qualquer valor opcional exibido (como `promptId`) deve existir explicitamente na assinatura da rotina (ou ser variável local declarada); nunca referenciar identificadores implícitos, para evitar `Compile error: Variable not defined` com `Option Explicit`.
- Em VBA (case-insensitive), evitar nomes de variáveis que colidam com palavras reservadas por casing (ex.: `eNum` ~ `Enum`); preferir nomes explícitos (`errNoCaptured`, `errDescCaptured`) para prevenir `Compile error: Syntax error`.
- Em assinaturas públicas que recebem `Type` (UDT) partilhado entre módulos (ex.: `ApiResultado`), evitar `ByVal`; usar `ByRef` para prevenir `Compile error: User-defined type may not be passed ByVal` em VBE.
- Em erros HTTP 400 com `code=context_length_exceeded`, registar evento dedicado com métricas de tamanho (`payload_len`, `prompt_len`, `input_array_len`, contagem de `input_text/input_file/input_image`, `max_output_tokens`) e mensagem didática de ação; evitar reportar apenas o body bruto sem contexto operacional.

- No validador bloqueante de anexos (`INPUTFILES_MISSING`), tratar `FILES:` com wildcard e `(latest)` como padrões resolvíveis para nome real no `inputFolder` antes da comparação com `filesUsed`; isto evita falso negativo sem perder bloqueio útil.
- Antes de enviar /v1/responses, validar contrato de anexos declarados em `INPUTS: FILES` contra o payload efetivo (input_file/input_image/text_embed) e bloquear com evento explícito (`INPUTFILES_MISSING`) quando faltar anexo esperado.
- Em fallback de `process_mode=code_interpreter` por listagem de container, registar detalhes por item (`filename`, `bytes`, `created_at`) no evento principal para evitar diagnósticos ambíguos com apenas totais.
...
- Quando a execução pode falhar antes da chamada HTTP (catálogo/config/files), registar breadcrumbs `STEP_STAGE` no DEBUG em pontos de fronteira do passo (entrada, before/after ContextKV, before/after anexação de INPUTS, parse config, files, before_api, api_call_start) para evitar casos "sem erro e sem Seguimento".
- Quando ocorrer exceção inesperada no fluxo principal da pipeline, tentar sempre registar fallback mínimo no `Seguimento` (com passo/prompt/stage) para evitar execução silenciosa sem auditoria.
- Ao auto-injetar `code_interpreter` por `Modos`, aplicar gate de intencao explicita quando houver `input_file/input_image`: sem `process_mode: code_interpreter` (ou `tool_choice` equivalente) no extra, suprimir a injecao e registar evento dedicado no DEBUG para evitar falsos erros de "/mnt/data".
- Em validação de intenção explícita de CI no M05, não depender apenas do fragmento JSON enviado ao /v1/responses quando `process_mode` for chave interna; cruzar também com o modo efetivo resolvido de File Output para evitar `M05_CI_AUTO_SUPPRESS` indevido.
- Em troubleshooting de CI com anexos, registar sempre o evento `M05_CI_INTENT_EVAL` com os campos `ci_in_extra`, `ci_intent_resolved` e `ci_explicit_intent` para tornar auditável a decisão de suprimir/ativar auto-add da tool.
- Em fallback de `process_mode=code_interpreter` por listagem de container, manter a whitelist de extensões alinhada com os marcadores de contrato textual (`FILE_TSV`, `OUTPUT_FILE`, etc.); ausência de `.tsv` na elegibilidade cria falso `File not found` apesar de artefacto válido.
- No resolvedor de artefactos de `process_mode=code_interpreter`, aplicar prioridade determinística `citation > CI_OUTPUT_FILE > fallback` e, no fallback, excluir sempre basenames `file-*` para evitar falso sucesso com ficheiros de input.
- No diagnóstico de seleção de artefactos CI por listagem de container, registar sempre motivo por item (`SIM`/`NAO`) sem depender de `bytes`; metadado `bytes` pode vir `null` e não deve bloquear tentativa de download por `file_id`.
...
- Antes de commitar, varrer módulos `.bas/.cls/.frm` por marcadores de conflito Git (`<<<<<<<`, `=======`, `>>>>>>>`); estes tokens no código/commentários de header geram `Compile error: Expected: line number or label or statement or end of statement` no VBE.
- Em validações de payload JSON (`contents_api` e similares), preferir leitura por parser (`GH_TreeCommit_JsonPick`/equivalente) para verificar presença de `sha/author/committer` em vez de `InStr` com literais de aspas; reduz risco de `Compile error: Syntax error` por escaping e de detecção parcial de chave.
...
- Sempre que uma chave de configuração enumera modos operacionais (ex.: `GH_UPLOAD_MODE=contents_api|tree_commit`), garantir dispatch explícito no entrypoint principal; não deixar valores "documentados" sem caminho de execução real no runtime.
- Em configurações com default operacional reportável (ex.: `GH_UPLOAD_MODE`), evitar aplicar fallback prematuro na fase de carga (`Config_Load`); resolver default na fase de decisão para preservar logs como `*_DEFAULTED`.
- Em leitura de segredos por variável de ambiente no VBA (ex.: `GH_TOKEN_ENV`), não depender apenas de `Environ$`; aplicar fallback de escopo `WScript.Shell.Environment("USER"/"SYSTEM")` e registar `token_source` acionável no DEBUG para diagnosticar sessões de Excel sem herança de ambiente.
- Em diagnósticos de token GH no DEBUG, incluir a origem com escopo (`ENV:<VAR>@PROCESS|USER|SYSTEM`, alias ou `CONFIG`) para distinguir ausência real de variável de sessão desatualizada do Excel.
...
- Em migração de schema de folhas de auditoria, quando os headers canónicos mudarem, aplicar reset controlado (`Cells.Clear`) e reformatar área de dados para evitar lixo visual herdado (ex.: fundo de header em rows de dados) e desalinhamento de colunas.
- Em folhas de auditoria com separadores visuais entre runs, a normalização idempotente não deve aplicar `.Interior/.RowHeight` em massa sobre a área inteira; iterar apenas rows com conteúdo para preservar separadores (ex.: row 6pt preta).
- Quando eventos tardios (ex.: upload) não recebem `runId` explícito, resolver `run_id` por cache/fallback da última run válida para manter rastreabilidade contínua no log.
