...
- Em rotinas com `GoTo` para controlo de fluxo local (filtros/loops), garantir que cada destino existe no mesmo procedimento e preferir labels descritivas (`NextRow`, `ContinueLoop`); validar antes do commit para evitar `Compile error: Label not defined`.
