...
'
' Atualiza̵̤es:
' - 2026-03-05 | Codex | Hardening de encoding URL (pend̻ncia)
'   - Substitui encoding byte-a-byte simplificado por percent-encoding UTF-8 por char.
'   - Mant̩m path-segments e owner/repo seguros para caracteres Unicode.
...
Public Function GH_TreeCommit_ContentsUrl( _
    ...
    GH_TreeCommit_ContentsUrl = normalizedBase & "/repos/" & _
                                GH_TreeCommit_UrlEncodeUtf8(owner) & "/" & _
                                GH_TreeCommit_UrlEncodeUtf8(repo) & "/contents/" & _
...
Private Function GH_TreeCommit_EncodePath(ByVal repoPath As String) As String
    ...
    For i = LBound(parts) To UBound(parts)
        parts(i) = GH_TreeCommit_UrlEncodeUtf8(parts(i))
    Next i

    GH_TreeCommit_EncodePath = Join(parts, "/")
End Function

Private Function GH_TreeCommit_UrlEncodeUtf8(ByVal value As String) As String
    Dim out As String
    Dim i As Long
    Dim ch As String
    Dim code As Long
    ...
    For i = 1 To Len(value)
        ...
        Select Case code
            Case 48 To 57, 65 To 90, 97 To 122, 45, 46, 95, 126
                out = out & ch
            Case Else
                out = out & GH_TreeCommit_PercentEncodeUtf8Char(ch)
        End Select
    Next i

    GH_TreeCommit_UrlEncodeUtf8 = out
End Function

Private Function GH_TreeCommit_PercentEncodeUtf8Char(ByVal oneChar As String) As String
    ...
    GH_TreeCommit_PercentEncodeUtf8Char = out
    Exit Function
Fallback:
    GH_TreeCommit_PercentEncodeUtf8Char = "%3F"
End Function
