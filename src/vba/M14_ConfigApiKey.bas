...
'
' AtualizaÃ§Ãµes:
' - 2026-02-16 | Codex | DiagnÃ³stico rÃ¡pido para Immediate Window
'   - Adiciona helper sem exposiÃ§Ã£o de segredo para validar resoluÃ§Ã£o da API key em runtime.
'   - Inclui cenÃ¡rios determinÃ­sticos para teste manual na Immediate Window.
...
' - Config_Diag_Immediate_Status() As String
'   - Devolve resumo textual (sem API key) para validaÃ§Ã£o na Immediate Window.
' - Config_Diag_Immediate_Scenarios() (Sub)
'   - Imprime cenÃ¡rios determinÃ­sticos de precedÃªncia/falha na Immediate Window.
' =============================================================================
...
Public Function Config_Diag_Immediate_Status() As String
    Dim apiKey As String
    Dim src As String
    Dim warnTxt As String
    Dim errTxt As String

    If Config_ResolveOpenAIApiKey(apiKey, src, warnTxt, errTxt) Then
        Config_Diag_Immediate_Status = "OK | source=" & src & " | key_len=" & CStr(Len(apiKey))
        If Trim$(warnTxt) <> "" Then
            Config_Diag_Immediate_Status = Config_Diag_Immediate_Status & " | alert=" & warnTxt
        End If
    Else
        Config_Diag_Immediate_Status = "ERRO | " & errTxt
    End If
End Function

Public Sub Config_Diag_Immediate_Scenarios()
    Dim apiKey As String
    Dim src As String
    Dim warnTxt As String
    Dim errTxt As String
    ...
    Debug.Print "[APIKEY] Scenario A (ENV + Config literal)"
    ok = Config_SelfTest_ResolveOpenAIApiKey("env-secret", "cfg-secret", apiKey, src, warnTxt, errTxt)
    Debug.Print "  ok=" & CStr(ok) & " | source=" & src & " | key_len=" & CStr(Len(apiKey)) & " | alert=" & warnTxt & " | error=" & errTxt

    Debug.Print "[APIKEY] Scenario B (somente Config literal)"
    ok = Config_SelfTest_ResolveOpenAIApiKey("", "cfg-secret", apiKey, src, warnTxt, errTxt)
    Debug.Print "  ok=" & CStr(ok) & " | source=" & src & " | key_len=" & CStr(Len(apiKey)) & " | alert=" & warnTxt & " | error=" & errTxt

    Debug.Print "[APIKEY] Scenario C (Config diretiva Environ sem ENV)"
    ok = Config_SelfTest_ResolveOpenAIApiKey("", "(Environ(""OPENAI_API_KEY""))", apiKey, src, warnTxt, errTxt)
    Debug.Print "  ok=" & CStr(ok) & " | source=" & src & " | key_len=" & CStr(Len(apiKey)) & " | alert=" & warnTxt & " | error=" & errTxt

    Debug.Print "[APIKEY] Scenario D (sem ENV e sem Config)"
