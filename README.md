...
- a tabela `GIT LOG` usa o formato canónico `Timestamp | Pipeline | PromptID | Version | Success | New version | Analysis Link | New Prompt Link | Eliminar | Summary`, com timestamp `yyyy-mm-dd hh:mm` e separador visual (row 6pt preta) entre runs diferentes.
- eventos de upload sem `runId` explícito reutilizam o `run_id` da run corrente para manter continuidade de rastreabilidade no `Summary`.
